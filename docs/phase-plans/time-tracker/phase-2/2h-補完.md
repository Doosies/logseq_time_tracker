...
- [x] `StorageManager.executeWithFallback()`: 런타임 쓰기 3회 재시도(500ms→1s→2s) 후 Memory 전환
- [x] `StorageManager.startAutoRecovery()`: 30초 주기 `tryRecover()` + 성공 시 자동 중지
- [x] `StorageManager.is_readonly`: 락 실패 시 읽기 전용 + 5초 재시도
- [x] `TimerService`: readonly guard로 쓰기 메서드 차단
- [x] `App.svelte`: fallback/readonly 배너 + 복구 토스트
- [x] `persistActiveTimerState()`: 실행 중 세그먼트 경과시간 포함한 `accumulated_ms` 저장
- [x] `2e-fallback.md`: Logseq beforeunload SDK 미지원 주석
- [x] `AppContext.dispose()`: timer → storage 순서로 정리
- [x] `main.ts`: `app_context.dispose()` 단일 호출
