...
2. 동일 작업에 대해 **3회 재시도**하며, 간격은 **exponential backoff**: 500ms → 1s → 2s.
3. 모든 재시도 실패 시:
...
- Logseq 환경에서는 `logseq.beforeunload` 훅을 우선 등록하고, 일반 웹 환경에서는 `window.beforeunload`를 보조로 사용하는 이중 등록을 검토합니다(실제 API는 플러그인 런타임에 맞게 선택).
...
- [ ] `StorageState` 상태 머신 구현
- [ ] 초기화 실패 → Memory fallback + 배너
- [ ] 런타임 실패 3회 → Memory fallback
- [ ] 복구: Memory → SQLite 데이터 마이그레이션
- [ ] Web Locks API 래퍼 구현
- [ ] Lock 실패 시 읽기 전용 모드
- [ ] `beforeunload` 처리(ActiveTimerState 저장, `accumulated_ms` 갱신, Sqlite 시 `persist`)
- [ ] 30초 주기 백업(`setInterval` + `dispose`에서 `clearInterval`)
- [ ] 사용자 알림 UI(배너, 토스트) 연동
