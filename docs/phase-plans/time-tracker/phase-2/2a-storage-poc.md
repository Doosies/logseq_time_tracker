...
- 초기화: 브라우저·`wasm_url` 지정 시 `loadWasmBinary(base_url)`로 `sql-wasm.wasm`을 XHR 후 `fetch` 폴백으로 로드한 뒤 `initSqlJs({ wasmBinary })` (제품·PoC 공통, `wasm_loader.ts`). Node 등에서는 인자 없이 `initSqlJs()` 폴백
- Vite에서 WASM 등 정적 자산 배치: `viteStaticCopy` 또는 `public/` 복사로 `sql-wasm.wasm`이 `wasm_url` 기준 경로에서 로드되도록 구성
...
- PoC 전용 DB명으로 `indexedDB.open('time-tracker-poc')` 가능 여부 (`poc_storage_test.ts`의 `POC_IDB_NAME`과 동일)
