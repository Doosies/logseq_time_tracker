...
### UF-16: Toolbar에서 작업 확인 및 제어

**전제**: 앱 초기화 완료

| 단계 | 사용자 행동                           | 시스템 결과                                              |
| ---- | ------------------------------------- | -------------------------------------------------------- |
| 1    | Logseq 툴바 아이콘 클릭              | Toolbar 드롭다운 표시                                    |
| 2    | -                                     | 진행 중 Job 정보 (타이틀, 경과 시간) 또는 빈 상태 표시  |
| 3    | "일시정지"/"완료" 클릭                | 사유 모달 표시 → 입력 → TimerService 호출 (UF-03~05 참조) |
| 3a   | 대기/일시정지 Job에 마우스 호버       | 호버 액션 버튼 표시 (시작/재개/전환, 완료, 취소)         |
| 3b   | 호버 액션 버튼 클릭                   | 해당 액션 실행 (사유 모달 포함)                          |
| 4    | 활성 작업 없을 때 "시작"/"재개" 클릭  | 새 타이머 시작 (UF-01 참조)                              |
| 5    | "풀화면 열기" 클릭                    | FullView 패널 열림 (UF-17)                               |

**빈 상태**: 활성 작업이 없으면 잡 목록에 시작/재개 버튼 표시, 드롭다운에 "활성 작업 없음" 메시지

> **테스트**: UC-UI-019~023 | **설계**: 06-ui-ux §1 Toolbar

---

### UF-17: 풀화면 UI 모드 전환 (Compact ↔ Full)

**전제**: 앱 초기화 완료

| 단계 | 사용자 행동                           | 시스템 결과                                              |
| ---- | ------------------------------------- | -------------------------------------------------------- |
| 1    | Toolbar "풀화면" 버튼 또는 단축키     | FullView 패널 열림                                       |
| 2    | -                                     | LayoutSwitcher가 ResizeObserver로 컨테이너 폭 감지       |
| 3    | (폭 < 600px)                          | Compact 모드: 탭 네비게이션 (작업 / 기록 / 설정)        |
| 4    | (폭 ≥ 600px)                          | Full 모드: 사이드바 + 메인 2컬럼 레이아웃                |
| 5    | 패널 크기 조절                        | 실시간 모드 전환 (200ms 디바운스)                        |
| 6    | "닫기" 버튼 또는 Escape 키            | FullView 닫기, Toolbar로 복귀                            |

> **테스트**: UC-UI-010 | **설계**: 06-ui-ux §2 FullView + 반응형 레이아웃
...
| 3     | UF-14~17 | 수동 TimeEntry, overlap 해결, 커스텀 필드, UI 모드 전환        |
| 4     | UF-18~19 | 템플릿, 알림/리마인더                                          |
| 5     | UF-20~21 | 통계 조회, 데이터 내보내기                                     |
