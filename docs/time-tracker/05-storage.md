...
**구현 참고 (Phase 2)**: `SqliteUnitOfWork`는 `IUnitOfWork` 계약상 `dataFieldRepo`가 필요하므로, Phase 2B DDL에 `data_field` 테이블이 없는 동안 **`SqliteDataFieldRepository` 스텁**(`adapters/storage/sqlite/sqlite_data_field_repository.ts`)을 둡니다. 본격적인 영속화는 Phase 3에서 DDL·쿼리와 함께 구현할 예정이며, 스텁은 `getDataFields`가 항상 빈 배열을 반환하고 `upsert`/`delete`는 no-op입니다.
...
실제 경로는 `packages/time-tracker-core/src/adapters/storage/sqlite/migrations/` 입니다 (문서상 상대 경로: `adapters/storage/sqlite/migrations/`).

```
adapters/storage/sqlite/migrations/
  001_initial.ts   # job, category, time_entry, job_history, settings 등 초기 스키마
  002_phase2.ts    # Phase 2 확장 (job_category, job_template, external_ref 등 — 구현 내 DDL 참고)
  index.ts         # ALL_MIGRATIONS 등록
```

> Phase 3 메타 레지스트리(`data_type`, `entity_type`, `data_field` 등)용 마이그레이션은 **아직 별도 파일로 분리되지 않았으며**, 도입 시 `003_*.ts` 형태로 이 디렉터리에 추가하는 것을 전제로 합니다.
...
// types/export.ts 기준 (테이블별 배열은 data 하위에 둠)
...
```typescript
// packages/time-tracker-core/src/services/data_export_service.ts 패턴과 동일
...
`beforeunload` 이벤트(플러그인 코드에서는 `window.addEventListener('beforeunload', …)`)에서:

> **iframe 제한**: Logseq 플러그인은 iframe 내에서 실행됩니다. `beforeunload`는 **메인 윈도우 이벤트**이므로 iframe에서 직접 감지하지 못할 수 있습니다. 이를 보완하기 위해 30초 주기적 백업을 fallback으로 병용합니다.[^storage-beforeunload]

[^storage-beforeunload]: 과거 문서에서는 `logseq.beforeunload`를 전제로 한 적이 있으나, **`@logseq/libs` SDK에는 해당 API가 없습니다.** 종료 전 정리는 `window.addEventListener('beforeunload', …)`만 사용합니다. (아키텍처 문서: [02-architecture.md §10](02-architecture.md), [§14](02-architecture.md).)
...
1. 앱 시작 시 SqliteUnitOfWork 초기화 시도
   - 초기화 재시도: StorageManager 기본 `runWithExponentialBackoff`는 `base_delay_ms: 50`, `max_attempts: 3`이며,
     실패 후 대기 간격은 50ms, 100ms입니다 (`base_delay_ms * 2^attempt`; 패턴상 다음 단계는 200ms).
2. 실패 시 (StorageError):
...
3. 재시도 시:
   ...
   c. 실패 시 → 배너 유지, 사용자에게 "데이터보내기" 권장
4. 런타임 중 SQLite 쓰기 실패 시 (앱 시작은 정상이었지만 이후 오류) — **아직 구현되지 않음** (`2h-補完.md` 등에서 제안):
   a. 제안: 3회 재시도, `base_delay_ms: 500` 기준 지수 백오프로 **500ms, 1s, 2s**
...
`StorageState` 타입은 **`types/settings.ts`** 에 정의되어 있으며, 런타임 전환·구독은 `adapters/storage/storage_state.ts` 의 `StorageStateMachine` 이 담당합니다.

```typescript
// types/settings.ts (발췌)
