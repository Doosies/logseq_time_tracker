...
MR이 없는 환경에서는 하나의 작업이 여러 커밋으로 나뉘어 푸시되는 경우가 많다. 이를 개별로만 보면 `"wip"`, `"fix typo"`, `"done"` 같은 커밋 메시지끼리의 **작업 단위 연결**이 검색·후처리에서 끊긴다. 수집 직후 연속 커밋을 **논리적 변경 단위(Logical Change Unit)**로 식별하되, 커밋을 합쳐 하나의 diff/요약으로 만들지는 않는다. **diff를 합산하면 크기가 폭증**할 수 있으므로, 각 커밋은 독립적으로 LLM 요약하고 `group_id` / `group_size` / `group_index` **메타데이터로만** 그룹을 표현한다.
...
| ----------- | ----------------------------------------- | --------------------------------------- |
| 동일 작성자 | `author` 일치                             | 필수                                    |
| 시간 간격   | **시간순으로 인접한** 두 커밋 간 ≤ N분   | 기본값: 30분 (`grouping.time_gap_minutes`) |

인제스트 입력이 브랜치·기간 단위로 주어지면, 그 범위 안에서만 위 규칙이 적용된다(실질적으로 동일 브랜치·동일 수집 배치 내 연속 커밋).

선택적 강화 조건(설계상, 구현에서는 비활성 가능):

| 조건      | 기준                      | 비고          |
| --------- | ------------------------- | ------------- |
| 파일 겹침 | 변경 파일이 1개 이상 겹침 | 정확도 향상용 |
...
```typescript
/** 그룹 감지 결과의 개념적 표현. 커밋 본문은 합치지 않는다. */
interface LogicalChangeUnit {
    group_id: string; // 그룹의 가장 오래된 커밋 hash
    commits: string[]; // 포함된 커밋 hash 목록 (시간순, 오래된 것 먼저)
    author: string;
    time_range: {
...
| ------------ | --------------------------------- | ------------------------------------------------------------- |
| 단독 커밋    | 커밋 1건당 1회 LLM 호출           | CommitDocument 1개, **그룹 메타데이터 없음**                  |
| 다중 커밋 그룹 | 각 커밋을 **독립** 요약 (동일 수의 문서) | CommitDocument N개, 각 metadata에 `group_id` 등으로 연결 |

그룹에 속한 커밋의 경우:

- 각 커밋의 구조화 팩트·**해당 커밋의 diff만** LLM에 전달 (통합 diff 없음)
- `group_id` = 그룹의 가장 오래된 커밋 hash, `group_size` = 그룹 내 커밋 수, `group_index` = 그룹 내 순서(0-based)
- 검색으로 특정 커밋을 찾은 뒤, 동일 `group_id`로 필터하면 같은 작업 흐름의 인접 커밋을 함께 조회할 수 있다

**CommitDocumentMetadata** (그룹 필드, 단독 커밋이면 생략):

```typescript
group_id?: string; // 그룹의 가장 오래된 커밋 hash
group_size?: number; // 그룹 내 커밋 수
group_index?: number; // 그룹 내 순서 (0-based)
```
...
| 등급        | 기준                         | 전략     | 처리 방식                                      |
| ----------- | ---------------------------- | -------- | ---------------------------------------------- |
| normal      | ≤ `normal_max_lines`(기본 5,000줄) | `full`   | 전체 diff를 LLM에 전달                         |
| oversized   | > `normal_max_lines`         | `sample` | 파일별 대표 hunk 샘플링 후 LLM에 전달          |

**토큰 추정 로직** (2-tier 분기):

```typescript
function estimateDiffSize(diff_lines: string[], normal_max_lines = 5000): DiffSizeGate {
    ...
    if (total_lines <= normal_max_lines) {
        tier = 'normal';
        strategy = 'full';
    } else {
        tier = 'oversized';
...
> `normal_max_lines`는 `.cursor/cr-rag-mcp.yaml`의 `diff_size_gate.normal_max_lines`로 조정. `cost_per_token`은 모델 변경 시 갱신.

**파일별 대표 hunk 샘플링** (`strategy === 'sample'`):

- 통합 diff 문자열을 `diff --git` 헤더 기준으로 **파일 단위**로 분리한다.
- 전체 샘플링 **줄 예산**을 정한 뒤, 파일 수로 나누어 파일당 기본 할당량(`budget / file_count`)을 둔다.
- 각 파일에서 **변경 라인(`+`/`-`)이 있는 hunk**를 중심으로 대표 구간을 추출한다(문맥 몇 줄 포함).
- 할당량을 다 쓰지 못한 파일이 있으면 남은 budget을 **변경량이 큰 파일 순**으로 재배분한다.
- `truncateDiff` 같은 단순 앞부분 절단은 사용하지 않고, `sampleDiffByFile` 등으로 파일별 샘플을 구성한다.
...
diff_size_gate:
    normal_max_lines: 5000 # normal 이하: full, 초과: sample
```
...
interface DeduplicationRule {
    ...
    same_logical_group: boolean; // 동일 group_id → 맥락 조합·랭킹 보조 (커밋별 문서는 유지)
}
```
