...
#### UC-VRT-004: Toolbar에서 전환 클릭 시 ReasonModal 최상위 표시 및 포인터 클릭

- **Given**: 잡 2개가 존재하고, 하나가 시작되어 활성 타이머가 있는 상태이다
- **When**: 다른 잡의 전환 버튼을 클릭하여 ReasonModal이 표시된 상태에서 스크린샷을 캡처한다
- **Then**: ReasonModal이 전체 뷰포트를 올바르게 덮고, 베이스라인과 시각적 차이가 0.01 이내이다
- **And**: 확인 버튼을 정상 포인터 클릭으로 클릭하면 모달이 닫힌다 (z-index 정상 증명)
- **Phase**: 3
- **테스트 레벨**: VRT
