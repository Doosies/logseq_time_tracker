<!--
@component DatePicker - 달력 기반 날짜 선택 컴포넌트
-->
<script lang="ts">
    import { SvelteDate } from 'svelte/reactivity';
    import * as styles from '../../design/styles/date_picker.css';
    ...
    let { value, onSelect, min, max, locale = 'ko-KR', class: extra_class }: Props = $props();
    ...
    function getDayCellClass(cell: CalendarCell): string {
        const parts: string[] = [];
        parts.push(styles.day_cell);
        if (!cell.in_current_month) {
            parts.push(styles.day_outside_month);
        }
        if (cell.disabled) {
            parts.push(styles.day_disabled);
        }
        if (value === cell.date_str) {
            parts.push(styles.day_selected);
        }
        if (today_str === cell.date_str) {
            parts.push(styles.day_today);
        }
        return parts.join(' ');
    }
</script>

<div class={[styles.root, extra_class].filter(Boolean).join(' ')} bind:this={container_ref}>
    <button
        type="button"
    ...
    {#if is_open}
        <div class={styles.panel} role="dialog" aria-label="달력">
            <div class={styles.header}>
                ...
                <span class={styles.month_label}>{month_label_text}</span>
                <button
                    type="button"
                    class={styles.nav_button}
                    onclick={(e) => {
