...
skills:
  - .cursor/skills/system-improvement/SKILL.md
...
## 사용 가능한 Skill
- [System-Improvement SKILL](../skills/system-improvement/SKILL.md)

## Skill 참조 절차 (필수)

시스템 개선 작업 시작 전 아래 절차를 반드시 수행합니다.

1. `.cursor/skills/system-improvement/SKILL.md`를 선참조하여 분석 목표를 확정합니다.
2. 메트릭/병목/개선안/A-B 테스트 레퍼런스를 단계별로 선참조합니다.
3. 개선안 제출 전 레퍼런스 체크리스트 충족 여부를 교차 검증합니다.
4. 개선 리포트에 적용한 Skill 경로와 채택/롤백 근거를 기록합니다.
...
### 1단계: 메트릭 수집 및 분석
- **Skill 사용**: `.cursor/skills/system-improvement/references/performance-monitoring.md`
- `.cursor/metrics/cycles/` 내 모든 사이클 데이터 읽기
- 에이전트별, 워크플로우별 통계 계산
- 추세 분석 (시간에 따른 변화)

### 2단계: 병목 지점 식별
- **Skill 사용**: `.cursor/skills/system-improvement/references/bottleneck-analysis.md`
- 가장 느린 단계 찾기 (시간 병목)
...
### 3단계: 개선안 작성
- **Skill 사용**: 
  - `.cursor/skills/system-improvement/references/rule-optimization.md` - agents/*.md 파일 수정
  - `.cursor/skills/system-improvement/references/skill-generation.md` - Skill 추가/수정
  - `.cursor/skills/system-improvement/references/workflow-optimization.md` - 워크플로우 재설계
- **3계층 지식 분리 원칙 준수**:
...
### 4단계: A/B 테스트 설계
- **Skill 사용**: `.cursor/skills/system-improvement/references/ab-testing.md`
- Control 그룹: 기존 방식
- Treatment 그룹: 새로운 방식
- 각 그룹 10개 사이클
- 측정 지표 정의
...
- 메트릭 분석 → `.cursor/skills/system-improvement/references/performance-monitoring.md`
- 병목 식별 → `.cursor/skills/system-improvement/references/bottleneck-analysis.md`
- 에이전트 파일 수정 → `.cursor/skills/system-improvement/references/rule-optimization.md`
- Skill 생성 → `.cursor/skills/system-improvement/references/skill-generation.md`
- 워크플로우 변경 → `.cursor/skills/system-improvement/references/workflow-optimization.md`
- A/B 테스트 → `.cursor/skills/system-improvement/references/ab-testing.md`
