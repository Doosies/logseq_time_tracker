...
### 검색·랭킹·후처리(스코어링 파이프라인) 설계 시 (필수)

유사도, 시간 감쇠, 가중 합성 등 **복합 스코어**를 쓰는 검색·RAG·추천 파이프라인을 설계할 때는 아래를 설계 문서에 **명시**합니다.

- [ ] **필터(컷·임계값·하드 드롭)에 쓰는 지표**: 예) 원시 `similarity_score` 또는 별도 `min_relevance` — 어떤 값으로 “탈락”시키는지 한 줄로 정의
- [ ] **정렬·Top-K에 쓰는 지표**: 예) `final_score = similarity × time_decay` — 랭킹 전용인지 명시
- [ ] **분리 원칙**: “합성·감쇠된 스코어로 필터링하면 의미상 관련 있는 후보가 잘릴 수 있음” 등, 필터와 랭킹을 **다른 지표로 쓰는 이유**를 한 문단 이내로 기술 (해당 시)
- [ ] **단계 순서**: 검색 → (선택) 필터 → (선택) 재랭킹 → 응답, 각 단계의 입력/출력 필드명을 표로 정리

**근거**: 설계에 “시간 가중·후처리”만 있고 필터/정렬 구분이 없으면, 구현에서 동일 복합 스코어를 필터와 정렬에 동시에 써 회귀 버그가 나기 쉬움 (과거 사이클 사례).
...
Chrome Extension에서 설정 저장소를 설계/변경할 때는 아래를 요구사항에 **명시적으로 포함**해야 합니다:

1. **FOUC 방지 2단계 초기화** 명세 (동기+비동기 함수 분리, 호출 위치)
2. **마이그레이션 명세** (갱신 상태 목록 표: persist + 메모리 + UI)
3. **첫 설치 vs 기존 사용자 시나리오** (각각 1줄 이상 명세)
4. **보안 권고 처리** (파일 입력 시 크기·형식 제한)

- 범용 패턴: `.cursor/skills/developer/references/chrome-extension-storage.md`
- 프로젝트 지식은 `.cursor/skills/project-knowledge/references/`에서 필요 시 로드
