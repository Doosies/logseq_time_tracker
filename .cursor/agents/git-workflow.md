...
커밋 메시지 생성 시 아래 절차를 반드시 수행합니다.

1. **사전 참조 (필수)**  
   `.cursor/skills/git-workflow/references/commit-message-generation.md`를 먼저 읽고 Type/Scope/Subject 규칙을 적용합니다.
2. **생성 규칙 적용 (필수)**  
   Type/Scope는 레퍼런스의 결정 로직을 기준으로 선택하고, Subject는 명령형/50자 이내/마침표 생략 규칙을 따릅니다.
3. **체크리스트 교차 검증 (필수)**  
   레퍼런스 문서의 체크리스트(형식/내용/검증)를 기준으로 생성 결과를 검토합니다.
4. **근거 기록 (필수)**  
   변경 사항 요약 리포트에 Type/Scope 선택 근거를 함께 기록합니다.
...
- 또는: 사용자 승인 시 `GIT_COMMANDS.md`의 명령을 메인 에이전트가 실행 가능
...
| Type | 설명 | 예시 |
|---|---|---|
| `feat` | 새 기능 | `feat(auth): JWT 토큰 갱신 추가` |
| `fix` | 버그 수정 | `fix(api): null 응답 처리 보완` |
...
- [ ] 스테이징된 변경 사항과 일치
- [ ] 명확하고 구체적
- [ ] `commit-message-generation.md` 체크리스트(형식/내용/검증) 충족
- [ ] Type/Scope 선택 근거가 리포트에 기록됨
...
- **커밋 메시지 생성** → `.cursor/skills/git-workflow/references/commit-message-generation.md` (반드시 선참조)
- **PR 설명 생성** → `.cursor/skills/git-workflow/references/pr-description-generation.md`
- **변경 사항 분석** → `.cursor/skills/git-workflow/references/change-analysis.md`
- **리뷰어 추천** → `.cursor/skills/git-workflow/references/reviewer-recommendation.md`
...
- [ ] Conventional Commits 형식 준수
- [ ] `commit-message-generation.md` 기준 체크리스트 검증 완료
- [ ] 스테이징된 변경 사항과 커밋 메시지 일치
- [ ] PR 설명에 모든 필수 섹션 포함 (PR 생성 시)
- [ ] 관련 이슈 자동 연결 완료 (해당 시)
- [ ] 리뷰어 추천 완료 (PR 생성 시)
