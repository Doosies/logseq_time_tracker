...
MCP 서버 개발 시작 전 아래 절차를 반드시 수행합니다.

1. `.cursor/skills/developer/SKILL.md`를 선참조하여 구현 원칙을 확정합니다.
2. MCP 설계/구현/테스트/배포 레퍼런스를 단계별로 선참조합니다.
3. 구현 완료 전 레퍼런스 체크리스트 충족 여부를 교차 검증합니다.
4. 완료 리포트에 적용한 Skill 경로와 성능 개선 근거를 기록합니다.
...
### 6단계: Skill 업데이트
```markdown
// .cursor/skills/qa/references/coverage-check.md 업데이트
...
### 1단계: 패턴 감지 및 분석
- **Skill 사용**: `.cursor/skills/developer/references/mcp/pattern-detection.md`
- 메트릭 데이터에서 반복 패턴 식별
- 빈도, 소요 시간, 에러율 측정
- 자동화 가능성 평가

### 2단계: 도구 갭 분석
- **Skill 사용**: `.cursor/skills/developer/references/mcp/tool-gap-analysis.md`
- 현재 도구로 해결 불가능한 작업 파악
- 느린 작업, 에러 많은 작업 식별
- 우선순위 결정 (Impact vs Effort)

### 3단계: MCP 서버 설계
- **Skill 사용**: `.cursor/skills/developer/references/mcp/mcp-design.md`
- 도구 이름 정의 (명확하고 간결하게)
- 입력 파라미터 스키마 정의
- 출력 형식 정의 (JSON)
- 에러 케이스 정의
...
### 4단계: 구현
- **Skill 사용**: `.cursor/skills/developer/references/mcp/mcp-implementation.md`
...
### 5단계: 테스트
- **Skill 사용**: `.cursor/skills/developer/references/mcp/mcp-testing.md`
- 단위 테스트 작성 (Vitest)
- 통합 테스트 작성
- E2E 테스트 (실제 Cursor에서)

### 6단계: 배포 및 등록
- **Skill 사용**: `.cursor/skills/developer/references/mcp/mcp-deployment.md`
- 빌드: `npm run build`
...
- 패턴 감지 → `.cursor/skills/developer/references/mcp/pattern-detection.md`
- 도구 갭 → `.cursor/skills/developer/references/mcp/tool-gap-analysis.md`
- 설계 → `.cursor/skills/developer/references/mcp/mcp-design.md`
- 구현 → `.cursor/skills/developer/references/mcp/mcp-implementation.md`
- 테스트 → `.cursor/skills/developer/references/mcp/mcp-testing.md`
- 배포 → `.cursor/skills/developer/references/mcp/mcp-deployment.md`
