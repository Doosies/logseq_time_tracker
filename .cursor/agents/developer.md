...
### 스코어링·후처리(필터·랭킹) 구현 시 (필수)

검색·RAG·추천 등에서 유사도, 시간 감쇠, 가중 합성 스코어를 구현할 때:

1. **설계 문서에서 필터 지표 vs 정렬 지표를 구분해 읽기**
    - 필터(임계값·컷)와 Top-K 정렬에 **동일한 복합 스코어**를 쓰는 것이 의도인지 확인
    - 설계에 구분이 없으면 메인/기획에 질문한 뒤 진행 (임의로 `final_score`를 필터에 쓰지 말 것)

2. **구현 규칙 (기본)**
    - 관련성 컷은 **원시 또는 설계에 명시된 기준 지표**(예: `similarity_score`)로 하고, 시간 감쇠 등은 **랭킹 단계**에 두는 패턴을 기본으로 검토
    - 설계가 명시적으로 “복합 스코어로 필터”를 요구하는 경우에만 예외

**근거**: 복합 스코어를 필터에 사용하면 의미상 관련 있는 결과가 탈락하는 회귀가 발생할 수 있음.
...
- “나중에 테스트”로 테스트 인프라를 비워 두지 말 것 — 버그는 경계 조건에서 재발하기 쉬움
