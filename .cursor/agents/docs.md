...
### 다중 설계 문서·파이프라인 문서 동기화 (필수)

코드 변경이 **데이터 파이프라인, 검색·RAG, 스코어링, MCP 도구 동작** 등에 영향을 줄 때:

- [ ] 해당 기능의 **번호·절 목차가 있는 설계 문서**(예: `01-*.md`, `02-data-pipeline.md`)를 README·인덱스 또는 리포 내 링크로 찾아 **전부 목록화**
- [ ] 필터 기준·정렬 기준·단계 순서·필드명 등 **행동이 바뀌는 문장**이 있는 파일은 빠짐없이 갱신 (한 파일만 고치고 나머지가 구버전을 가리키는 상태 방지)
- [ ] 동일 개념(`similarity_score`, `final_score` 등)의 정의가 **모든 관련 문서에서 일치**하는지 확인

**근거**: 소스 한 곳 수정에 설계 문서 여러 개가 동시에 어긋나면 다음 구현자가 잘못된 스펙을 따름 (cycle 2026-03-23-007 사례).
