...
- `type/scope`는 Conventional Commits 표준 키워드 사용 (`feat`, `fix` 등)
- **Subject는 한글로 작성**
- **Body는 한글로 작성**
...
| Type | 설명 | 예시 |
|------|------|------|
| `feat` | 새 기능 추가 | `feat(auth): JWT 토큰 갱신 추가` |
...
1. **한글 작성**: Subject는 한글로 작성
2. **동사 명확화**: "추가", "수정", "개선", "정리" 등으로 의도 명확화
3. **50자 이내**: 간결하고 명확하게
4. **마침표 생략**: 끝에 마침표 없음
5. **구체적**: 무엇을 변경했는지 명확히
...
```markdown
feat(auth): JWT 토큰 갱신 추가
fix(api): null 응답 처리 보완
docs(readme): 설치 가이드 업데이트
refactor(service): 공통 검증 로직 분리
perf(api): 인덱스 기반 쿼리 성능 개선
```

### ❌ 나쁜 예

```markdown
feat(auth): JWT 토큰 갱신 기능을 추가했습니다.  # 문장형 서술 지양
fix: 버그 수정  # 너무 모호함
docs: 업데이트  # 무엇을 업데이트했는지 불명확
feat: 기능 추가  # scope 없음, subject 모호
feat(auth): 사용자 경험 향상을 위한 JWT 토큰 갱신 기능 추가  # 50자 초과
```
...
```markdown
revert: revert "feat(auth): JWT 토큰 갱신 추가"
