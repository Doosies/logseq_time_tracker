...
- **0~8단계**: 각 단계별 서브에이전트 활용 (7단계 시스템 개선은 메인 에이전트 직접 수행). 9단계(최종 보고서)만 메인 에이전트가 직접 제출.
- **메인 에이전트 금지**: 0~8단계의 실제 작업(코드 작성, 커밋, 테스트 실행 등) 직접 수행 금지. 반드시 해당 서브에이전트 호출. 단, 7단계(시스템 개선)는 메인 에이전트가 스킬 참조하여 직접 수행.
...
7. **시스템 개선** ← 메인 에이전트 직접 수행 (system-improvement 스킬 참조)
    - `.cursor/skills/system-improvement/SKILL.md` 스킬 참조
